...
DCQCN 需要交换机和 RNIC 配合，配置分工如下：

- **交换机侧**配置 RoCE 队列、ECN 门限和标记概率，负责产生 CE 标记；
- **接收端 RNIC**识别 CE，并按照配置生成 CNP；
- **发送端 RNIC**启用 DCQCN，根据收到的 CNP 调整发送速率。

DCQCN 通常不配置“每次固定降低 20%”这样的比例。发送端根据 CNP 更新拥塞程度并动态计算降速幅度；没有继续收到 CNP 时，再通过快速恢复、加性恢复等阶段逐步提高速率。

生产环境应优先使用网卡厂商针对具体 RNIC、固件和驱动版本提供的推荐配置。只有出现队列长期不回落、PFC 频繁触发或发送速率明显振荡时，才需要结合 ECN、CNP 和队列数据调整参数。

### 3. DCQCN 为什么不能完全取代 PFC？
