...
ODP 需要 RNIC、驱动和内核共同支持，首次缺页可能增加延迟；显式 ODP 仍注册一个地址范围，只有隐式 ODP 才使用可覆盖整个进程地址空间的特殊 MR。
...
RQ 则用于提前准备接收资源，主要服务 Send/Recv，以及需要远端接收通知的 Write with Immediate。多个 QP 也可以按需共享一个 SRQ，也就是 Shared Receive Queue。

### 5. CQ：Completion Queue，完成队列

RDMA 操作是异步的。

应用提交请求后不会站在原地等网卡搬完，而是继续做别的事，之后再从 CQ 查看结果。
...
Receive WR 完成时会产生 CQE。对于 SQ 上成功完成的 WR，只有 QP 配置为全部通知，或该 WR 设置了 `IBV_SEND_SIGNALED`，才会产生成功 CQE；错误和 Flush 还会产生相应错误完成。
...
RDMA 最容易混淆的地方，是 Send/Recv、Write 和 Read 看起来都像“发数据”。
...
`ibv_post_send()` 返回成功，只表示 WR 已经成功进入工作队列，不表示网络操作已经完成。

真正处理 CQ 时，还要检查 `WC.status`：
...
对于未请求成功 CQE 的 unsignaled SQ WR，不能因为“没有 CQE”就立即复用 Buffer。常见做法是让后续一个 signaled WR 生成完成，再依据同一 SQ 的顺序关系批量回收此前的请求。

无论哪一种硬件完成，都不等于业务已经确认、连接故障后仍具备 exactly-once、GPU 已经消费，或数据已经持久化到存储介质。
...
这里的可靠与按序，是单个 QP 的传输层语义。它不提供连接故障后的业务 exactly-once，也不替代跨 QP、CPU/GPU 可见性和持久化顺序；Atomic 能力还取决于设备支持。

AI 通信、HPC 和存储场景里，大量数据通路都会使用 RC。

### 2. UC：不可靠连接

UC 也是一对一连接，但不做 ACK 和重传，不保证可靠交付。
...
此外还有 XRC、DC 等面向连接规模和资源共享的机制，但不影响理解 RDMA 的基本主线。
...
在多机大模型训练中，上层框架通常不会让每个用户手写 Verbs。
...
不是。完成语义取决于操作类型；业务消费、GPU 执行和存储持久化都需要上层协议另行确认。
