...
![GDS 的三种数据路径](assets/gpudirect-storage/01-gds-three-paths-v3.png)
...
如果应用准备的显存暂时不适合直接接收数据，cuFile 就可能使用这种路径。

可以把它理解成收快递：能直接送到家就直接送；暂时不能送到家，就先放到附近的中转站，再转给你。cuFile 的内部 GPU buffer 就是这个“显存中转站”。
