...
> 阅读提示：第一次接触 RDMA，只要先看懂三件事：数据为什么能少绕路、MR/QP/CQ 分别做什么、Send/Write/Read 有什么区别。RoCEv2 的网络配置与调优，会在后续文章单独展开。
...
RDMA 的思路是：应用把任务交给支持 RDMA 的网卡，由网卡在两台主机的已注册内存之间搬运数据。这样可以缩短数据快路径，减少不必要的 CPU 参与。
...
- RDMA：支持 RDMA 的网卡通过网络，直接读写另一台主机已经授权的内存。
...
| 关键词 | 用大白话解释 |
|---|---|
| Zero-copy | 尽量减少应用缓冲区与内核缓冲区之间的额外复制 |
| Kernel bypass | 数据快路径通常不必每次都经过内核协议栈 |
| Transport offload | 队列处理、权限检查、分包和部分传输工作由 RNIC 完成 |
...
进行单边操作时，对端通常需要 remote_addr、rkey 和长度。初学时只要理解为：“远端把一块有权限的货架位置告诉了我”。
...
| Write with Immediate | Write 数据，同时附带一条通知 | 是，接通知 | 是 |
...
| 类型 | 入门理解 |
|---|---|
| RC（Reliable Connection） | 一对一、可靠且有序；常用于 AI、HPC 和存储 |
...
在 RC 模式下，QP 常见状态会从 RESET 进入 INIT、RTR、RTS。知道 RTS 表示“可以发送”即可，具体参数在真正写 Verbs 程序时再学习。

---

## 七、RDMA 的性能取舍：高速度背后的资源成本

RDMA 把部分成本从“每条消息都经过内核”换成了“提前准备与正确管理资源”：

- 内存注册有开销，实际程序通常复用已注册的 Buffer，而不是每次传输都重新注册。
...
第一次看图时，只看中间的软件栈和服务器之间的网络路径即可：训练框架调用通信库，通信库使用 RDMA，RDMA 再使用 IB 或 RoCE 网络。
