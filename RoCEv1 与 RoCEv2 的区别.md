...
![RoCE 为什么需要 PFC、ECN 与端侧拥塞控制](assets/roce-v1-v2/05-roce-loss-congestion-control.png)

这套保护机制可以分成三个环节：

1. **ECN 负责提前预警。** 当交换机发现队列开始积压时，它不会立即丢包，而是给经过的 IP 报文设置 CE 标记。
2. **CNP 和端侧拥塞控制负责主动降速。** 接收端 RNIC 收到带 CE 标记的报文后，向发送端返回 CNP。发送端的拥塞控制算法据此降低注入速率，让进入网络的流量逐渐与可用带宽匹配。
3. **PFC 负责接近溢出时的最后保护。** 如果队列继续增长并达到 PFC 门限，交换机会向直接相连的上游设备发送 Pause 帧，暂时停止指定优先级的流量，尽量避免缓冲区溢出。
...
需要特别注意，PFC 和 ECN/CNP 的作用并不相同。PFC 是逐跳暂停机制，只作用于直接相邻的链路；ECN/CNP 则把拥塞信息反馈给流量源，让发送端主动调整速率。PFC 配置不当还可能造成队头阻塞、拥塞扩散甚至 PFC 风暴，因此不能把“开启 PFC”简单等同于完成 RoCE 拥塞控制。实际部署需要统一设计 PFC 门限、ECN 门限、交换机缓存以及网卡的端侧拥塞控制算法。

一句话概括：**PFC 尽量避免队列溢出和丢包；ECN + CNP 则让真正的拥塞源主动降速。**
