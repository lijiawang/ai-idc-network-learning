...
GRH 是 Global Route Header（全局路由头），来自 InfiniBand 协议，长度固定为 40 字节。它包含源 GID、目的 GID、流标签、流量类别和跳数限制等字段，结构和作用类似 IPv6 头。不过，在 RoCEv1 中，GRH 并不是供普通以太网交换机执行 IP 路由的外层网络头；交换机仍然根据以太网帧的 MAC 地址进行二层转发。

因此，RoCEv1 可以在同一个二层网络或 VLAN 内运行，但不能像普通 IP 报文一样，通过三层路由器跨越不同子网。
...
Leaf-Spine 网络中，两台服务器之间通常有多条都能到达目的地、并且路由代价相同的路径。ECMP 的作用，就是从这些等价路径中选择一条。交换机通常根据源 IP、目的 IP、UDP 源端口、UDP 目的端口和协议类型计算哈希，再由哈希结果决定经过哪台 Spine。

可以简单理解为：**先找出多条都能到达目的地的等价路径，再从中选一条。** 同一条流的哈希结果通常不变，所以会固定走同一条路径，以减少报文乱序；不同的流可能走不同 Spine，从而并行利用多条链路。需要注意，ECMP 通常不会判断哪条链路当前最空闲。多条大流仍可能碰巧被选到同一路径上，形成哈希碰撞和局部拥塞。对于 RoCEv2，UDP 目的端口通常固定为 `4791`，协议类型也固定为 UDP，因此不同流之间的哈希差异主要来自源/目的 IP 和承载流标识的 UDP 源端口。
