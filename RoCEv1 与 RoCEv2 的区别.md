...
RoCE 对丢包和拥塞比较敏感，因此实际部署中经常配合 PFC、ECN 和端侧拥塞控制使用。

ECN 是显式拥塞通知。当交换机发现队列开始积压时，可以给报文设置 ECN 标记。接收端收到被标记的报文后，通过 CNP 通知发送端降低速率：
...
PFC 则是逐跳流量暂停机制。当某个优先级队列接近溢出时，交换机可以要求上游设备暂停发送该优先级的流量，从而减少丢包。

需要特别注意：PFC 不是端到端拥塞控制。配置不当时，它还可能造成队头阻塞、拥塞扩散甚至 PFC 风暴。因此，PFC、ECN、交换机缓存和网卡拥塞控制算法需要统一设计。
